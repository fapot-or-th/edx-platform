...
def update_certificate_available_date_on_course_update(self, course_key):
    ...
    if (course_overview and course_overview.self_paced is False):
        ...
        # this is an invalid update so let's log something and maybe we can understand our problems a bit better
        else:
            LOGGER.warning(
                f"Skipping update to the course certificate configuration in Credentials of course run [{course_key}]. "
                f"An invalid instructor-paced course run configuration was detected with a display behavior of "
                f"[{course_overview.certificates_display_behavior}], a certificate available date of "
                f"[{course_overview.certificate_available_date}], and a course end date of [{course_overview.end_date}]"
            )
