...
@skip_unless_lms
@mock.patch('openedx.core.djangoapps.programs.tasks.update_certificate_visible_date_on_course_update')
...
class CoursePacingChangedReceiverTest(TestCase):
    ...
    def test_handle_course_pacing_change_instructor_paced(
        self,
        ...
        """
        Test that verifies the behavior of the `handle_course_pacing_change` signal receiver when a signal is simulated
        for an instructor-paced course run.
        """
    ...
    def test_handle_course_pacing_change_self_paced(
        self,
        mock_is_creds_enabled,
        mock_cad_task,
        mock_visible_date_task
